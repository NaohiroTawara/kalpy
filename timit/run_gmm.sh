...
# Kaldi scripts for TIMIT dataset 
# Based on the code: 
...
if [ $# -ne 2 ]; then
  echo "Usage: $0 <TIMIT-dir> <KALDI-dir>"
  echo "e.g.: $0 /data2/tawara/work/db/timit /data2/tawara/work/kaldi"
  exit 1;
fi
timit=$1
kaldi=$2
...
if [ ! -d local ] ; then
    ln -s $kaldi/egs/timit/s5/local ./
fi
if [ ! -d utils ] ; then
    ln -s $kaldi/egs/timit/s5/utils ./
fi
if [ ! -d steps ] ; then
